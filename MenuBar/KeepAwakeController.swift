...
/// Holds a system/display idle-sleep assertion so the Mac stays awake while enabled.
/// Owned as a process-wide singleton so closing the panel does not drop the assertion.
final class KeepAwakeController: ObservableObject {
    ...
    func start() {
        ...
        let reason = "Menu Bar keep-awake" as CFString
        ...
        if running {
            ...
            log.info("keep-awake on (IOKit display=\(self.displayAssertion, privacy: .public) idle=\(self.idleAssertion, privacy: .public) caffeinate=\(self.caffeinateProcess?.isRunning == true, privacy: .public))")
        } else {
    ...
    private func startCaffeinateFallback() {
        ...
        do {
            ...
            log.info("caffeinate fallback started (pid \(process.processIdentifier, privacy: .public))")
        } catch {
