...
def write_pbxproj() -> None:
    ...
    project_debug = """
		...
		SWIFT_VERSION = 5.0;
"""
    project_release = project_debug.replace("DEBUG_INFORMATION_FORMAT = dwarf;", 'DEBUG_INFORMATION_FORMAT = "dwarf-with-dsym";')
    project_release = project_release.replace("ENABLE_TESTABILITY = YES;", "ENABLE_NS_ASSERTIONS = NO;")
    project_release = project_release.replace("GCC_DYNAMIC_NO_PIC = NO;\n\t\t\t\tGCC_NO_COMMON_BLOCKS = YES;\n\t\t\t\tGCC_OPTIMIZATION_LEVEL = 0;\n\t\t\t\tGCC_PREPROCESSOR_DEFINITIONS = (\n\t\t\t\t\t\"DEBUG=1\",\n\t\t\t\t\t\"$(inherited)\",\n\t\t\t\t);\n", "GCC_NO_COMMON_BLOCKS = YES;\n")
    project_release = project_release.replace("MTL_ENABLE_DEBUG_INFO = INCLUDE_SOURCE;", "MTL_ENABLE_DEBUG_INFO = NO;")
    project_release = project_release.replace("ONLY_ACTIVE_ARCH = YES;\n\t\t\t\t", "")
    project_release = project_release.replace(
        'SWIFT_ACTIVE_COMPILATION_CONDITIONS = "DEBUG $(inherited)";\n\t\t\t\tSWIFT_OPTIMIZATION_LEVEL = "-Onone";',
